...
- 解释执行（javascript）：解释一行，执行一行，由浏览器负责解释成cpu能识别的，速度相对较慢，要来切换翻译、执行。它的好处在于灵活性-动态特性，随时可以给一个对象添加属性和方法。java等不行。

- 编译执行（java、c#）：它是一次性将所有的代码翻译成cpu能识别的01010，然后在一行一行去执行。

- 函数是 JavaScript 中的一等重要的。在java中是类，函数是次重要的。
...
- Web APIs
  - BOM
    - onload页面加载事件，window顶级对象，所有的内容都在window对象下
    - 定时器
    - location、history
  - DOM
    - 获取页面元素，注册事件
...
此时先把域名由运营商的DNS（域名解析为IP地址）服务器转换成IP地址，DNS再告诉浏览器这个IP地址，浏览器再根据这个IP地址发送请求，告诉服务器，要去访问你的首页，服务器把内容准备好之后，返回给浏览器。内容就是一些字符串，就是html文档内容，包涵html代码，css代码，js代码，然后浏览器中的部件去分别解析这些内容

```
User Interface  用户界面，我们所看到的浏览器，包括工具栏，页面的可视区
Browser engine  浏览器引擎，用来查询和操作渲染引擎
*Rendering engine 渲染引擎 用来显示请求的内容，负责解析HTML、CSS，并把解析的内容显示出来，并渲染成一棵DOM树，是在内存中存储的
Networking 在浏览器输入www.baidu.com按下回车，它在网络上发送一个请求，负责发送请求和接收来自服务器返回的内容，接收到的基本是字符串
*JavaScript Interpreter(解析者)   JavaScript解析器，负责解析、执行JavaScript的代码，操作内存中的DOM树，然后渲染引擎重新去更新这个DOM树，再告诉浏览器引擎已经更新完毕，把显示在用户界面
UI Backend   UI后端，用来绘制类似组合框和弹出窗口，如alert、
...
JavaScript 运行分为两个阶段，不是真正的从上往下去执行：
...
严格来讲，这就相当于说对象是一组没有特定顺序的值。对象的每个属性或方法都有一个名字，而每个名字都映射到一个  值。
...
![img](file://C:/Users/ASUS/Desktop/html_study/%E7%AC%94%E8%AE%B0/JavaScript%E9%AB%98%E7%BA%A7/media/664ba37eeee9f4623c06c066867f1d38_r.jpg?lastModify=1618209835)
...
如果面向过程中出错，是比较难找的，要重新看下整个程序的流程，面向对象，只要看哪个对象出了问题，去找对应的构造函数就ok
...
```javascript
var person = new Object() // 它目前是一个空对象，里面没有任何的属性、方法
person.name = 'Jack'
person.age = 18
...
每次创建通过 `new Object()` 比较麻烦，所以可以通过它的简写形式对象字面量 {} 可以看成是new Object()缩写来创建：
...
但却没有解决对象识别的问题（即怎样知道一个对象的具体类型，只是知道是object，使用instance时候无法知道）。
...
1、构造函数名字的第一个字母大写，用于区分普通函数

new 之后会产生四个步骤

（1）会在内存中创建一个空的对象

（2）设置构造函数中的this指针，指向刚刚创建好的对象

（3）执行构造函数中的代码

（4）返回刚刚创建的对象

无法使用typeof获取对象的具体类型，返回的都是object

获取对象的具体类型，对象有一个constructor属性，不建议使用，它是可以被修改的。
...
直接通过对象来调用的属性和方法，称为静态成员（一般是字面量方式）。没有动态成员

工具中使用静态成员，如Math，可以求出最大值，最小值等等，这个Math工具只有一个



当有很多个对象的时候，使用构造函数来创建

实例成员：构造函数中的成员就是实例成员，它的调用需要创建出实例之后，才能通过实例去调用
...
```javascript
console.log(p1.sayHello === p2.sayHello) // => false，它比较的其实是两个函数的地址，它们的内存地址是不一样的
```
...
function sayHello = function () {
  console.log('hello ' + this.name) // 在书写函数的时候，this的指向是无法确认的，只有当它被调用，执行的时候，才能确定，书写或者注册是暂时无法确定的。
}
...
JavaScript 规定，每一个构造函数都有一个 `prototype`（原型 / 原型对象） 属性，它其实是一个对象。
...
通过该构造函数创建的对象，可以访问构造函数.prototype中的成员（属性、方法）
```
...
console.log(stu1.toString());自己创建的对象返回的是固定的形式[object Object]，object代表的是一个对象，Object代表的是类型，它获取的不是具体的类型，而是所有对象的祖宗
...
Array、Sring等内置对象的prototype属性，系统不允许属性的值。可以通过给它添加新的方法。
